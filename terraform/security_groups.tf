...
resource "aws_security_group" "backend" {
  ...
  description = "Backend ECS task -accept traffic only from ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "frontend" {
  ...
  description = "Frontend ECS task -accept traffic only from ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS -accept Postgres connections only from backend tasks"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "redis" {
  ...
  description = "ElastiCache Redis -accept connections only from backend tasks"
  vpc_id      = aws_vpc.main.id
